...
// #include <limits> //in order to check for the number of decimal digits
...
int main(void){
    ...
    ofstream result("result.txt"); //(creating and) opening the output file into the variable <result>
    ...
    if(data.is_open()){
        ...
        like this, if you want to read it on the standard output, uncomment these next two lines: */
        ...
        while(!data.eof()){
            ...
            const long double a = -(7.0/4.0); /* I couldn't find online if the type "double" is promoted to "long double" 
        ...
        // Let's print an error that let the user know what to change in order to make the code work
        cerr << "File not found: you need a file named data.txt in the same directory of the executable in order to make the code work." << endl;
    }
    return 0;
    }